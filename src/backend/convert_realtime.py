...
def test(config):
    if config.use_url == False:
        ...
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        wav = load_wav_url(config.url_in)
        ...
        G.load_state_dict(torch.load(G_path, map_location=lambda storage, loc: storage))
        # get reference file
        src_spk_stats = np.load(join(config.train_data_dir, f'{config.src_spk}_stats.npz'))
        trg_spk_stats = np.load(join(config.train_data_dir, f'{config.trg_spk}_stats.npz'))
        ...
        # convert
        f0, timeaxis, sp, ap = world_decompose(wav, fs=sampling_rate, frame_period=5) # 5ms*16kHz = 80samples/frame, nframes = CHUNK/80 = 1000 
        ...
        sf.write(join(config.url_out, 'output.wav'), wav_transformed, sampling_rate)
...
if __name__ == '__main__':
    ...
    # Directories.
    parser.add_argument('--train_data_dir', type=str, default='data/codedApSp_belt56_a/train')
