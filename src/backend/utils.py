...
def world_encode_aperiodic(ap, fs):
...
def world_speech_synthesis(f0, coded_sp, coded_ap, fs, frame_period, feat):
    if feat == 'apsp':
        coded_sp_real = coded_sp[:, :-4]
        coded_ap = coded_sp[:, -1]  # last col
        assert(coded_sp_real.shape[1] == 56)
        fftlen = pyworld.get_cheaptrick_fft_size(fs)
        coded_sp_real = np.ascontiguousarray(coded_sp_real)
        decoded_sp = world_decode_spectral_envelop(coded_sp_real, fs)
        coded_ap = np.expand_dims(coded_ap, axis=1)
        decoded_ap = pyworld.decode_aperiodicity(
            coded_ap.astype(np.float64), fs, fftlen)
    elif feat == 'ap' or feat == 'sp':
        fftlen = pyworld.get_cheaptrick_fft_size(fs)
        #coded_ap = np.expand_dims(coded_ap, axis=1)
        decoded_ap = pyworld.decode_aperiodicity(
            coded_ap.astype(np.float64), fs, fftlen)
        ...
        assert False, "Not a valid feature"
    # TODO
    min_len = min([len(f0), len(decoded_sp), len(decoded_ap)])
    f0 = f0[:min_len]
    decoded_sp = decoded_sp[:min_len]
    decoded_ap = decoded_ap[:min_len]

    wav = pyworld.synthesize(f0, decoded_sp, decoded_ap, fs, frame_period)
...
def coded_ap_statistics(coded_sps):
    # sp shape (T, D)
    aps_concatenated = np.concatenate(coded_sps, axis = 0)
    aps_mean = np.mean(coded_aps_concatenated, axis = 0, keepdims = False)
    aps_std = np.std(coded_aps_concatenated, axis = 0, keepdims = False)
    return coded_sps_mean, coded_sps_std
