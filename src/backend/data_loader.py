...
spk2acc = {'262': 'Edinburgh',  # F
           ...
           '248': 'India',  # F
           '251': 'India'}  # M
# Since we slice 256 frames from each utterance when training.
min_length = 256
# Build a dict useful when we want to get one-hot representation of speakers.
speakers = ['chest', 'falset']
...
class MyDataset(data.Dataset):
    ...
    def sample_seg(self, feat, sample_len=min_length):
        ...
        s = np.random.randint(0, feat.shape[0] - sample_len + 1)  # TODO
    ...
    def __getitem__(self, index):
        ...
        # (T, D) -> (D, T), since pytorch need feature having shape
        mc = np.transpose(mc, (1, 0))
        # to one-hot
        spk_cat = np.squeeze(to_categorical(
            [spk_idx], num_classes=len(speakers)))
...
class TestDataset(object):
    ...
    def __init__(self, data_dir, wav_dir, src_spk='belt', trg_spk='breathy'):
        ...
        self.mc_files = sorted(
            glob.glob(join(data_dir, '*_{}_*.npy'.format(self.src_spk))))  # MODIFY!!

        self.src_spk_stats = np.load(join(data_dir.replace(
            'test', 'train'), '{}_stats.npz'.format(src_spk)))
        self.trg_spk_stats = np.load(join(data_dir.replace(
            'test', 'train'), '{}_stats.npz'.format(trg_spk)))
